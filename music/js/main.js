...
var createSentence = function() {
    var pNew = "";
    sentence = lyric[index+1].words;
    var sentence = lyric[index+1].words.map(function(item) {
        return item.data;
    }).join(" ");
    pNew += sentence;
}

var song = `<p>Hoa cỏ lau</p>
            <p>Ca sĩ: Hoàng An</p>`;

//listen event timeupdate
audio.addEventListener("timeupdate", function() {
    var currentTime = Math.floor(audio.currentTime * 1000);

    var index = lyric.findIndex(function(sentence) {
        sentence = sentence.words;
        return currentTime >= sentence[0].startTime && currentTime <= sentence[sentence.length-1].endTime;
    });
    //Hiển thị 2 câu đầu tiên nếu lời trước 5s
    ...
    if(index !== -1) {
        ...
        //     }).join(" ");
        //     pNew += sentence;
        //     var arr = Array.from(karaokeContent.children);
        //     arr[0].innerText = pNew;
        // } else {
        //     var pNew = "";
        //     sentence = lyric[index+1].words;
        //     var sentence = lyric[index+1].words.map(function(item) {
        ...
        //     pNew += sentence;
        //     var arr = Array.from(karaokeContent.children);
        //     // arr[1].classList.add("blur");
        //     arr[1].innerText = pNew;
        //     // arr[0].classList.add("blur");
        // }
        
       

    } else {
        karaokeContent.innerHTML = song;
    }
...
var handleColor = function(currentTime) {
    ...
    Array.from(sentencesEL).forEach(function(sentence) {
        ...
        if(startTime && endTime && currentTime > startTime) {
            //So sánh currentTime với StartTime của từng từ --> Biết được đang hát tới từ nào trong 1 câu
            ...
            /*Tính thời gian của từ với time bắt đầu câu */
            var time = endTimeResult - startTime;
            var rate = (time * 100) / sentenceTime;
            sentence.children[0].style.width = `${rate}%`;
            sentence.children[0].style.transition = `width ${result.endTime - result.startTime}ms linear`;
...
sreen 1: index = 0, index = 1
sreen 2: index = 2, index = 3
sreen 3: index = 4, index = 5
